...
namespace LazyAtHome.Web.Wash2.App_Code.Proxy
{
    public class UserProxycs
    {
        ...
        {
            var rtn = WCFInvokeHelper<UserClient>.Invoke<ReturnEntity<IList<User_CardDC>>>(c => c.Proxy.Web_User_Card_SELECT_List(userid));
            return rtn;
        }
        ...
        public static ReturnEntity<bool> Update_UserModifyMobile(Guid userid, string mpno)
        {
            var rtn = WCFInvokeHelper<UserClient>.Invoke<ReturnEntity<bool>>(c => c.Proxy.User_MPNo_Change(userid, mpno));
            return rtn;
        }


        public static ReturnEntity<bool> Update_UserModifyPass(Guid userid, string oldPass, string newPass)
        {
            var rtn = WCFInvokeHelper<UserClient>.Invoke<ReturnEntity<bool>>(c => c.Proxy.User_LoginPassword_Change(userid, oldPass, newPass));
            return rtn;
        }
