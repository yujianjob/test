var IMG = function(BOX,LEFT,RIGHT,WIDTH,HHUM,PLAY,SPEED,AOUT){
	...
	var Start = function (){
		if(_Index < 0){
			...
			BOX.stop(false,true).stop().animate({'left':'-'+l_width+'px'}, 0);
			_Index = (_count) - 1;
		}else if (_Index >= (_count)+1){
			BOX.stop(false,true).stop().animate({'left':0+'px'}, 0);
			_Index = 1;
		}
		move();
	}
	var move = function (){
		...
		BOX.stop(false,true).stop().animate({'left':_target+'px'}, SPEED);
