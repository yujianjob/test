...
namespace LazyAtHome.Web.Wash2.App_Code.Proxy
{
    public class OrderProxy
    {
        ...
        public static ReturnEntity<Order_OrderDC> Order_Onekey_Create(Guid iUserID, int iSite, Channel iChannel, Order_ConsigneeAddressDC iGetAddress, Order_ConsigneeAddressDC iSendAddress, DateTime? iExpectTime, string iUserRemark, int? iUserCouponID, int iUseMoney)
        {
            var rtn = WCFInvokeHelper<OrderClient>.Invoke<ReturnEntity<Order_OrderDC>>(c => c.Proxy.Order_Onekey_Create(iUserID, iSite, iChannel, iGetAddress, iSendAddress, iExpectTime,iUserRemark, iUserCouponID, null, null, null, iUseMoney ));
            return rtn;
        }
