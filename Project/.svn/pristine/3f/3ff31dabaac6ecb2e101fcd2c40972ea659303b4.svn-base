...
namespace LazyAtHome.Web.WeiXin
{
    public class CommonApi
    {
        ...
        /// <returns></returns>
        public static Models.JsonResult.WeixinUserInfoResult GetUserInfo(string accessToken, string openId)
        {
            var url = string.Format("http://api.weixin.qq.com/cgi-bin/user/info?access_token={0}&openid={1}",
                                    accessToken, openId);
            var result = Get.GetJson<Models.JsonResult.WeixinUserInfoResult>(url);
            return result;
        }
